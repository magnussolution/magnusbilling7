...
class ICEPAY
{
	...
	*/
	public function __construct( $merchantID = NULL, $secretCode = NULL )
	{
		...
		if ( $this->merchantID == NULL ) throw new Exception( "Please provide a merchantID" );
		if ( $this->secretCode == NULL ) throw new Exception( "Please provide a secret code" );
	...
	*/
	static public function GetCoreClasses()
	{
		return array
		(
		 	'icepay.php',
	...
	*/
	...
	{
		foreach ( $collection as $item ) if ( $find == $item ) return true;
		return false;
	}
	...
	*/
	protected function getStreamMethod() {
		if (function_exists("fopen")) 				return "fopen";
		if (function_exists("curl_init")) 			return "curl";
		if (function_exists("file_get_contents")) 	return "file_get_contents";
		return "fopen";
	}
