Help.load({
    ...
    'user.prefix_local': 'Use it to allow your user to dial with their local format. Rules description: The rules are separated by comma, and each rule is composed by 3 values with a \'/\' to separat them. The fist value is the number to search for/match. The second number is what to put in place of what is found. The third is the total number length before changes. The first number can be a \'*\' to match any digit and add the second number at the beginning. Examples: 0/55/11,*/5511/8,*/5511/9. RULE 1 -> numbers that start with 0 and length is 11 digits, replace 0 with 55. RULE 2 ->  numbers that start with any digit and length is 8, add 5511 at the beginning. RULE 3 ->  numbers that start with any digit and length is 9, add 5511 at the beginning.',
    ...
    'sip.disallow': 'Disallow all codecs and then select the codecs available below to enable them to the user. G729 is not installed by default because it uses a commercial license.',
