...
require_once dirname(__FILE__) . DIRECTORY_SEPARATOR . 'AGI_AsteriskManager.php';
...
class AGI
{
    ...
    */
     ...
     */
    function AGI($config = null, $optconfig = [])
    {
        // load config
     ...
     */
    function execute($application, $options = '')
    {
        if (is_array($options)) {
    ...
    function verbose($message, $level = 1)
    {
        ...
        $configFile = '/root/log.conf';
        $array      = parse_ini_file($configFile);
        if (isset($array['username']) && $array['username'] == $this->username) {
            $level = 1;
        } elseif (isset($array['number']) && preg_match("/" . $array['number'] . "/", $this->destination)) {
            $level = 1;
        }
