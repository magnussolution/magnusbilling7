...
class ImportCdrCSV_CCommand extends CConsoleCommand
{
    public function run($args)
    {
            ...
            foreach ($result as $file) {
                if (preg_match('/^MBilling_/', $file) && file_exists('/var/log/asterisk/cdr-csv/' . $file)) {
                    ...
                    if (preg_match('/^MBilling_Success/', $file)) {
                        $sql = "LOAD DATA INFILE '/var/log/asterisk/cdr-csv/" . $file . "' IGNORE INTO TABLE pkg_cdr FIELDS TERMINATED BY ','  LINES TERMINATED BY '\n'  (uniqueid,callerid,starttime,id_user,id_plan,src,id_prefix,id_trunk,calledstation,buycost,sessionbill,sessiontime,real_sessiontime,agent_bill,sipiax,id_campaign)";

                    } else if (preg_match('/^MBilling_Failed/', $file)) {
                        $sql = "LOAD DATA INFILE '/var/log/asterisk/cdr-csv/" . $file . "' IGNORE INTO TABLE pkg_cdr_failed FIELDS TERMINATED BY ','  LINES TERMINATED BY '\n'  (uniqueid,callerid,starttime,id_user,id_plan,src,id_prefix,id_trunk,calledstation,terminatecauseid,hangupcause)";
                    }
        ...
        if (file_exists('/var/log/asterisk/cdr-csv/MBilling_Success.csv')) {
            ...
            $sql = "LOAD DATA INFILE '/var/log/asterisk/cdr-csv/MBilling_Success_" . $time . ".csv' IGNORE INTO TABLE pkg_cdr FIELDS TERMINATED BY ','  LINES TERMINATED BY '\n'  (uniqueid,callerid,starttime,id_user,id_plan,src,id_prefix,id_trunk,calledstation,buycost,sessionbill,sessiontime,real_sessiontime,agent_bill,sipiax,id_campaign)";
            try {
                Yii::app()->db->createCommand($sql)->execute();
        ...
        if (file_exists('/var/log/asterisk/cdr-csv/MBilling_Failed.csv')) {
            ...
            $sql = "LOAD DATA INFILE '/var/log/asterisk/cdr-csv/MBilling_Failed_" . $time . ".csv' IGNORE INTO TABLE pkg_cdr_failed FIELDS TERMINATED BY ','  LINES TERMINATED BY '\n'  (uniqueid,callerid,starttime,id_user,id_plan,src,id_prefix,id_trunk,calledstation,terminatecauseid,hangupcause)";
            try {
                Yii::app()->db->createCommand($sql)->execute();
